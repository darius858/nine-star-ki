...
const DATA = {
  ...
  "month_ranges":[
    ...
    {"label":"8 sept - 8 okt","start":{"month":9,"day":8},"end":{"month":10,"day":8},"sequences":{"9":[9,7,7],"8":[8,4,9],"7":[7,1,2],"6":[6,7,4],"5":[5,4,6],"4":[4,1,8],"3":[3,7,1],"2":[2,4,3],"1":[1,1,5]}},
...
function isBeforeNineKiNewYear(d) {
  ...
  const boundary = new Date(y, 1, 4); // 4 feb
  return d < boundary;
}
...
function Card({ title, n }) {
  return (
    <div>
      <h3 className="font-medium mb-2" style={{ color: BRAND.colors.metal }}>{title}</h3>
      <StarBadge n={n} />
      <div className="mt-3"><Properties n={n} /></div>
    </div>
  );
}
        ...
        {result && result.info ? (
          <main className="space-y-6">
            ...
            <section className="bg-white rounded-2xl shadow-sm ring-1 p-6" style={{ borderColor: BRAND.colors.base }}>
              ...
              <p className="text-sm" style={{ color: BRAND.colors.metal }}>
                {flowSummary(result.info.yearStar, result.info.monthStar, result.info.dayStar)}
              </p>
            </section>
          </main>
        ) : (
